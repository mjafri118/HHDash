...
class HHDashboard:
    def __init__(self, master):
        ...
        self.grab_setups(master)

        #REMOVE AFTER GRAB_SETUPS TESTING.
        ...
        #self.screen_width, self.screen_height = 360, 640 #REMOVE AFTER TESTING VERTICAL.
    ...
    #Grabs orientation of screen and padding
    def grab_setups(self, master):

        #Reads setups.txt for each line-separated value.
        file = open("setups.txt", "r")
        ...
        #Cleans up orientation for later.
        if orientation == "h\n":
            self.orientation = "h"
        if orientation == "v\n":
            self.orientation = "v"

        #Converts padding to integers.
        self.cpadx = int(padx)
        self.cpady = int(pady)
