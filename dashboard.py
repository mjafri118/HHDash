...
class MyFirstGUI:
    ...
    def create_canvas(self, master):
        ...
        self.w.config(bd= 0, highlightbackground = "red", highlightcolor = "red", background = "red")
        self.w.pack()
    ...
    def logos(self, master):
        #Gets the images.
        image = Image.open("HH_logo_red.jpg")
        ...
        self.new_h = float(new_h)

    #Will create title in between the two logos.
    def HH_title(self,master):


        remaining = int( float(self.screen_width) - 2.0*(float(self.new_w)) )  #pads it 15 pixels
